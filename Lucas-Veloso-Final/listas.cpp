...
void Listas::carregar(){
    ...
    {//Carregar Compras Salvas
        ...
        for(auto &mudar : entrada){
            ...
            ui->tabelaMostrarSalvosCompras->setRowCount(0);

            for(auto preencher : compras.toStdMap()){

                int linhas = ui->tabelaMostrarSalvosCompras->rowCount();

                ui->tabelaMostrarSalvosCompras->insertRow(linhas);
                ui->tabelaMostrarSalvosCompras->setItem(linhas, 0, new QTableWidgetItem(preencher.first));
    ...
    {//Carregar Tarefas Salvas
        ...
        for(auto &mudar : entrada){
            ...
            ui->tabelaMostrarSalvosTarefas->setRowCount(0);

            for(auto preencher : tarefas.toStdMap()){

                int linhas = ui->tabelaMostrarSalvosTarefas->rowCount();

                ui->tabelaMostrarSalvosTarefas->insertRow(linhas);
                ui->tabelaMostrarSalvosTarefas->setItem(linhas, 0, new QTableWidgetItem(preencher.first));
    ...
    {//Carregar Compras Template
        ...
        for(auto &mudar : entrada){
            ...
            ui->tabelaMostrarTemplateCompras->setRowCount(0);

            for(auto preencher : templatesCompras.toStdMap()){

                int linhas = ui->tabelaMostrarTemplateCompras->rowCount();

                ui->tabelaMostrarTemplateCompras->insertRow(linhas);
                ui->tabelaMostrarTemplateCompras->setItem(linhas, 0, new QTableWidgetItem(preencher.first));
    ...
    {//Carregar Tarefas Template
        ...
        for(auto &mudar : entrada){
            ...
            for(auto preencher : templatesTarefas.toStdMap()){

                int linhas = ui->tabelaMostrarTemplateTarefas->rowCount();

                ui->tabelaMostrarTemplateTarefas->insertRow(linhas);
                ui->tabelaMostrarTemplateTarefas->setItem(linhas, 0, new QTableWidgetItem(preencher.first));
...
void Listas::on_ExibirOcultos_clicked()
{
    for (int i = 0; i < ui->tabelaListaDeCompras->rowCount(); i++) {
        ...
        if (check->checkState()){
            oculto = false;
            ui->tabelaListaDeCompras->showRow(i);
        }
    }
...
void Listas::on_ExibirOcultos_Tarefas_clicked()
{
    for (int i = 0; i < ui->tabelaListaDeTarefas->rowCount(); i++) {
        ...
        if (check->checkState()){
            oculto = false;
            ui->tabelaListaDeTarefas->showRow(i);
        }
    }
}
